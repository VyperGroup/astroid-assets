...
function domContentLoaded(){pokiInited=true;console.log("DOM content loaded event received");let canvas=document.getElementById('canvas');canvas.addEventListener("contextmenu",stopContextMenu);if(!postRunDone){resizeCanvas(false);}
...
gameReadyToStart=true;JumpGame.showInterstitial({beforeShowAd:()=>{},afterShowAd:()=>{startGame();}})}
...
function adInterstitialShow(){simpleLogC("adInterstitialShow()");JumpGame.showInterstitial({beforeShowAd:()=>{},afterShowAd:()=>{Module.ccall("ad_interstitial_on_showed","v",["number"],[1]);}})}
function adRewardedLoad(){simpleLogC("adRewardedLoad()");setTimeout(()=>Module.ccall("ad_rewarded_on_loaded","v",["number"],[1]),100);}
function adRewardedShow(){simpleLogC("adRewardedShow()");pokiEnsureStop();JumpGame.showReward({beforeShowAd:()=>{},afterShowAd:()=>{Module.ccall("ad_rewarded_on_reward","v");Module.ccall("ad_rewarded_on_showed","v",["number"],[true]);}})}
function firebaseDeinit(){}
function currentTimeSecondsRound(){return Math.round(Date.now()/1000);}
JumpGame.startup("2i8wau4ziwrk6a6h4hvf845in33d4a8y");